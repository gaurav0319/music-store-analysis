...
city name & sum of all invoice details.
...
of all Rock music listeners. Return your list alphabatically by email 
starting with A?
...
dataset.write a query that returns the artist name count of the top 10 
rock bands?
...
Q8: Return all the track names that have a strong length longer than the 
...
(
	...
	ROW_NUMBER() over(partition by customer.country order by count(invoice_line.quanitity) desc) as rowno
	FROM invoice_line
